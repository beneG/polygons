...
TEST_F(YoloDetectorTest, DetectWithNoPolygons) {
    YoloDetector detector(model_config_, model_weights_, class_names_, 0.5f, 0.4f);
...
TEST_F(YoloDetectorTest, DetectionStructureValidation) {
    ...
    YoloDetector detector(model_config_, model_weights_, class_names_, 0.1f, 0.4f);
...
TEST_F(YoloDetectorTest, ConfidenceThresholdAffectsDetections) {
    ...
    polygons.push_back(CreatePolygonConfig(
        ...
        exchange_protocol::PolygonType::INCLUDE,
        1,
        {"person", "car", "dog", "cat", "bicycle", "truck", "bus", "motorcycle"}
    ));
    
    YoloDetector detector_low(model_config_, model_weights_, class_names_, 0.1f, 0.4f);
    YoloDetector detector_high(model_config_, model_weights_, class_names_, 0.9f, 0.4f);
